...
export function useTimer(settings: TimerSettings = defaultSettings) {
  ...
  const skip = useCallback(() => {
    const nextMode: TimerMode =
      state.mode === "work"
        ? state.currentSession % settings.sessionsBeforeLongBreak === 0
          ? "longBreak"
          : "shortBreak"
        : "work";
    ...
    if (state.mode === "work") {
      setState((prev) => ({
        ...
        currentSession:
          prev.currentSession >= settings.sessionsBeforeLongBreak
            ? 1
    ...
    if (state.isRunning) {
      intervalRef.current = setInterval(() => {
        setState((prev) => {
          ...
          if (
            prev.timeRemaining <= 300 &&
            prev.timeRemaining > 299 &&
          ...
          if (prev.timeRemaining <= 1) {
            ...
            if (prev.mode === "work") {
              const nextMode =
                prev.currentSession % settings.sessionsBeforeLongBreak === 0
                  ? "longBreak"
                  : "shortBreak";
