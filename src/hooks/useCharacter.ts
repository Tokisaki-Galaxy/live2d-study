...
export function useCharacter(
  ...
  // Store message separately to avoid unnecessary re-renders
  const [message, setMessage] = useState<string>(() =>
    getRandomMessage("happy"),
  );
  const [showBubble, setShowBubble] = useState(true);

  // Compute the current mood based on timer state
  const currentMood = useMemo((): CharacterMood => {
  ...
  // Character object is derived from computed mood and message
  ...
  const [config, setConfig] = useState<CharacterConfig>(() => {
    try {
      ...
      return saved
        ? { ...defaultCharacterConfig, ...JSON.parse(saved) }
  ...
  const updateConfig = useCallback((newConfig: CharacterConfig) => {
    ...
    // Create a version without the big blob data (modelData is extracted but unused to avoid saving it)
    const { modelData: _unusedModelData, ...rest } = newConfig;
    void _unusedModelData; // Explicitly mark as intentionally unused
    localStorage.setItem("character-config", JSON.stringify(rest));
  }, []);

  // Track the previous mood to detect changes
  const prevMoodRef = useRef<CharacterMood>(currentMood);

  // Update message when mood changes (using effect for side effects like showing bubble)
  // This pattern is intentional: we need to update derived state and trigger side effects
  // when the computed mood changes based on prop changes
  useEffect(() => {
    if (prevMoodRef.current !== currentMood) {
      prevMoodRef.current = currentMood;
      // eslint-disable-next-line react-hooks/set-state-in-effect -- Intentional: updating derived state on mood change
      setMessage(getRandomMessage(currentMood));
    ...
    const idleInterval = setInterval(
      () => {
        ...
        if (!isRunning || timerMode !== "work") {
          setMessage(getRandomEventMessage("idle"));
          setShowBubble(true);
  ...
  const showMessageFunc = useCallback((msg: string) => {
    setMessage(msg);
    setShowBubble(true);
    setTimeout(() => setShowBubble(false), 5000);
  }, []);
  ...
  const showEventMessage = useCallback((event: keyof typeof eventMessages) => {
    setMessage(getRandomEventMessage(event));
    setShowBubble(true);
    setTimeout(() => setShowBubble(false), 5000);
  }, []);
