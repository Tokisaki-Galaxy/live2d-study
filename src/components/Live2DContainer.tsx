...
// Type definition to extend window for PIXI
declare global {
  interface Window {
    PIXI: typeof PIXI;
  }
...
export const Live2DContainer: React.FC<Live2DContainerProps> = ({
  ...
  // Play mood motion function - defined with useCallback to ensure stable reference
  const playMoodMotion = useCallback(
    ...
    const loadModel = async () => {
      ...
      if (modelRef.current) {
        appRef.current.stage.removeChild(
          modelRef.current as PIXI.DisplayObject,
        );
      ...
      try {
        ...
        if (active && model && appRef.current) {
          appRef.current.stage.addChild(model as PIXI.DisplayObject);
