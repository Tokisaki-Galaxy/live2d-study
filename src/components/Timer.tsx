...
const modeConfig: Record<TimerMode, { label: string; color: string; gradient: string }> = {
  work: {
    label: 'Focus Time',
    color: 'text-rose-400',
    ...
    <div className="flex flex-col items-center">
      ...
      <div className="relative mb-6">
        ...
        >
          ...
          <defs>
            <linearGradient id="gradient" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor={mode === 'work' ? '#fb7185' : mode === 'shortBreak' ? '#34d399' : '#60a5fa'} />
              <stop offset="100%" stopColor={mode === 'work' ? '#fb923c' : mode === 'shortBreak' ? '#14b8a6' : '#818cf8'} />
            </linearGradient>
          </defs>
        </svg>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          <span className={`text-6xl font-bold tabular-nums tracking-tight ${config.color}`}>
            {formattedTime}
          </span>
      ...
      {totalSessions > 0 && (
        <div className="mt-4 text-white/40 text-xs">
          Total sessions completed: <span className="text-white/60 font-medium">{totalSessions}</span>
        </div>
      )}
    </div>
  );
