...
export class SpineLoader {
  ...
  static async loadSpineFromZip(zipBlob: Blob): Promise<Spine | null> {
    try {
      ...
      const atlasPath = validFilePaths.find(
        (path) => path.endsWith(".atlas") || path.endsWith(".atlas.txt"),
      );

      // Find the skeleton file (.json or .skel)
      const skeletonPath = validFilePaths.find(
        (path) =>
      ...
      if (!skeletonPath) {
        throw new Error(
          "No skeleton .json or .skel file found in the zip archive",
        );
      }
      ...
      for (const textureName of textureNames) {
        ...
        const texturePath = validFilePaths.find(
          (path) => path.endsWith(textureName) || path.includes(textureName),
        );
      ...
      const skeletonBlob = new Blob([skeletonData], {
        type: skeletonPath.endsWith(".json")
          ? "application/json"
      ...
      return new Promise((resolve, reject) => {
        ...
        const textureResources = Object.entries(textureUrls).map(
          ([name, url]) => ({
            name,
            url,
          }),
        ...
        textureLoader.load(() => {
          ...
          loader.add("spineData", skeletonUrl, {
            metadata: {
              ...
              imageLoader: (
                _loaderParam: PIXI.Loader,
                resource: { name: string },
                resolveTexture: (baseTexture: PIXI.BaseTexture) => void,
              ) => {
