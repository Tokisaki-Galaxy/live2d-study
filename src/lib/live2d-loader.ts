...
export class Live2DLoader {
  ...
  static async loadModelFromZip(zipBlob: Blob): Promise<Live2DModel | null> {
    try {
      ...
      const modelJsonPath = validFilePaths.find(path => 
        path.endsWith('model3.json') || 
        path.endsWith('model.json')
      );

      if (!modelJsonPath) {
        throw new Error('No model3.json or model.json found in the zip archive');
      }

      // 2. Extract all files to Blobs and mapped paths
      // We need to maintain relative directory structure for resources
      const rootDir = modelJsonPath.substring(0, modelJsonPath.lastIndexOf('/') + 1);
      ...
      // 3. Process the Entry JSON
       ...
       const getBlobUrlForPath = async (relativePath: string) => {
         ...
         const cleanRelative = relativePath.startsWith('./') ? relativePath.substring(2) : relativePath;
       ...
       const configBlob = new Blob([patchedConfigStr], { type: 'application/json' });
